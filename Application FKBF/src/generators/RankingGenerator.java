...
import classement.FileChooser;
import classement.IntegerComparator;
import classement.ModeleModifiable;
import classement.ReadAndPrintXMLFile;
...
public class RankingGenerator extends JFrame {
	...
	public void reloadTableDB() {
		...
		Classement classement = FactoryDAO.getClassementDAO().selectAllClassement(getDateLong(), isHomme.isSelected() ? "M" : "F",
	...
	public void saveTableDB() {
		modele.orderTeams();
		Classement classement = new Classement(division.getText(), modele.getTeams(), getDateLong(), isHomme.isSelected());
	...
	private String getDateString() {
		...
		return (dateJournee.getModel().getYear() + "-" + (dateJournee.getModel().getMonth() + 1) + "-" + dateJournee.getModel().getDay());
	}
	...
	private void checkAll() {
		...
		for (Team team : teams) {
			...
			if (sumPoints != Integer.parseInt(team.getPoints().replace("-", "0"))) {
				errors.append(team.getTeam() + " : " + team.getPoints() + " différent de " + sumPoints + RN);
				error = true;
			}

			if (sumMatchs != Integer.parseInt(team.getMj().replace("-", "0"))) {
				errors.append(team.getTeam() + " : " + team.getMj() + " différent de " + sumMatchs + RN);
				error = true;
			}
	...
	private StringBuilder createHtml(Classement classement) {
		...
		html.append("		<td>Première place</td>" + RN);
		html.append("		<td>Deuxième place</td>" + RN);
		html.append("		<td>Troisième place</td>" + RN);
		html.append("		<td>Forfait</td>" + RN);
		html.append("		<td>Nb périodes</td>" + RN);
